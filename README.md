...
- ✅ 当前支持：Python 的安装、版本管理和依赖操作，Node.js 的安装、版本管理和全局 npm 包管理，以及 MeetAI 本体更新
...
这一节拆成两条最常见路径：
- Windows：下载 `meetai.exe`，放到固定目录并加入 PATH
- Linux x86_64：下载 `meetai`，加执行权限后放到 PATH 中

### 你需要准备什么？

- 💻 Windows 10 / 11，或 Linux x86_64 机器
- 🌐 能访问互联网（下载需要）
- ⏰ 5 分钟时间
...
> ⚠️ **安全提示**：请只从上面的 GitHub Releases 页面下载，不要使用陌生网盘、群文件或别人转发的可执行文件。Windows 如果提示"未知发布者"，请先确认下载来源；学校机房、团队分发或对安全要求更高时，建议按下面的可选步骤校验 sha256。

**第 3 步：可选，校验下载文件**

如果你同时下载了 `meetai.exe.sha256`，可以在 PowerShell 中运行：

```powershell
cd "$env:USERPROFILE\Downloads"
$file = "meetai.exe"
$expected = (Get-Content "$file.sha256").Split(" ")[0].Trim()
$actual = (Get-FileHash $file -Algorithm SHA256).Hash.ToLowerInvariant()
if ($actual -ne $expected) {
    throw "sha256 校验失败，请重新从 GitHub Releases 下载。"
}
...
常见终端都可以，例如 Ubuntu 的 Terminal、Debian 的 GNOME Terminal、KDE Konsole，或你常用的 shell 环境。

**第 2 步：下载 MeetAI**

```sh
cd /tmp
curl -L -O https://github.com/meetai-club/meetai/releases/latest/download/meetai

# 可选：同时下载 meetai.sha256 后校验
curl -L -O https://github.com/meetai-club/meetai/releases/latest/download/meetai.sha256
sha256sum -c meetai.sha256
```

如果你不想手写链接，也可以直接在 GitHub Releases 页面下载 `meetai`。如果下载了 `meetai.sha256`，可以先完成校验再继续。

**第 3 步：放到固定目录**

```sh
mkdir -p "$HOME/.local/bin"
cp meetai "$HOME/.local/bin/meetai"
chmod +x "$HOME/.local/bin/meetai"
```

**第 4 步：加入 PATH**

如果你的 `~/.local/bin` 还没在 PATH 中，可以加入 shell 配置：
...
> 无需安装 Rust，直接下载对应平台的二进制即可使用。

**Windows**

1. 从 [GitHub Releases](https://github.com/meetai-club/meetai/releases) 下载最新的 `meetai.exe`
2. 可选：同时下载 `meetai.exe.sha256` 并校验
3. 放到专属目录，例如 `D:\MeetAI\bin\meetai.exe`
4. 把 `D:\MeetAI\bin` 加入用户 PATH

**Linux x86_64**

1. 从 [GitHub Releases](https://github.com/meetai-club/meetai/releases) 下载 `meetai`
2. 可选：同时下载 `meetai.sha256` 并校验
3. 加执行权限后放到固定目录，例如 `~/.local/bin/meetai`
4. 确保 `~/.local/bin` 已加入 PATH
...
# Node.js 全局 npm 包管理
meetai npm install eslint                 # 安装到当前 Node.js 版本的全局空间
meetai npm list                           # 查看当前 Node.js 版本的全局 npm 包
meetai npm migrate --from 20.11.1 --to 22.0.0 # 迁移顶层全局 npm 包
...
# MeetAI 本体更新
meetai update check                       # 只检查是否有新版本
meetai update                             # 下载并应用新版本
```
...
构建产物位置：
- Windows: `target/release/meetai.exe`
- Linux/macOS: `target/release/meetai`

GitHub Releases 当前提供的构建产物：
- Windows: `meetai.exe`、`meetai-v<version>-windows-x86_64.exe`、`meetai-v<version>-windows-x86_64.zip`
- Linux x86_64: `meetai`、`meetai-v<version>-linux-x86_64`、`meetai-v<version>-linux-x86_64.tar.gz`
...
- ✅ `npm prefix/list` 在切换 Node.js 后可用
...
```powershell
meetai update check   # 只检查是否有新版本
meetai update         # 下载最新 Release，校验 sha256 后更新
```

自动更新目前支持 Windows x86_64 与 Linux x86_64。MeetAI 会下载对应平台的 `meetai.exe` / `meetai` 以及同名 `.sha256` 文件，校验通过后才替换当前可执行文件。

Windows 上如果当前 `meetai.exe` 正在运行导致无法立即覆盖，命令会生成一个延迟替换脚本，并提示你关闭当前命令后运行该脚本完成更新。

### 手动更新

1. 从 [GitHub Releases](https://github.com/meetai-club/meetai/releases) 下载对应平台的新版本
2. 可选：同时下载 `.sha256` 并校验；校验不一致时不要覆盖旧版本
3. Windows 用户覆盖 `meetai.exe`；Linux x86_64 用户替换 `meetai`
4. 如果安装路径没变，PATH 通常无需重新配置
