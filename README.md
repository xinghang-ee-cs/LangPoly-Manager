...
- 今天，`meetai` 已支持 Python / Node.js 运行时安装与版本管理，并为 Java / Go 预留统一入口
...
- 入口命令：`meetai`
- 当前定位：开发环境统一管理 CLI（当前已支持 Python 与 Node.js）
- MVP 范围（当前版本）：`Python runtime + pip + venv + quick-install` 主链路可用，`Node.js runtime` 已接入统一入口
- 当前重点：持续打磨 Python / Node.js 体验，并逐步补齐 Java / Go

## 版本状态（MVP）

当前版本已达到 **Python + Node.js 场景可用版本**：

1. 可用能力
- Python：安装 / 切换 / 卸载
- Node.js：列表 / 安装（Windows）/ 切换 / 卸载
- pip：安装 / 卸载 / 升级 / 列表
- venv：创建 / 激活提示 / 列表
- quick-install：Python 一键初始化（可选 venv，可选 Node.js）

2. 仍在开发
- Java / Go：统一入口已预留，自动安装与版本切换尚未完成
...
2. Node.js
- 运行时列表/切换/卸载：已支持
- 自动安装：Windows 已支持；macOS / Linux 暂不支持自动下载
- quick-install 参数：`--install-nodejs` 已支持（Windows 自动安装）
...
# 安装并切换 Node.js（Windows 自动安装）
meetai runtime install nodejs latest
...
```powershell
# 基于 meetai 可执行文件目录，当前会话立即生效
$meetaiExeDir = Split-Path -Parent (Get-Command meetai).Source
$env:Path = "$meetaiExeDir\.meetai\shims;$env:Path"

# 基于 meetai 可执行文件目录，持久生效（用户级，执行后重开终端）
[Environment]::SetEnvironmentVariable("Path", "$meetaiExeDir\.meetai\shims;" + [Environment]::GetEnvironmentVariable("Path", "User"), "User")

# 若你设置了 MEETAI_HOME，请改为 "$env:MEETAI_HOME\shims"
```
...
# 多语言参数入口（Node.js 在 Windows 支持自动安装；Java / Go 当前会给出“手动安装”提示）
...
下面这组命令用于验证当前 **Python 主链路 + Node.js 可选能力** 是否可用。  
...
8. （可选）验证 Node.js 主链路（Windows）
```powershell
cargo run -- runtime install nodejs latest
# 按上一条命令输出的实际版本号替换 <version>
...
2. `<meetai.exe目录>/.meetai`
3. 用户主目录：`~/.meetai`（仅在无法解析可执行文件目录时回退）
...
5. `python use` 更新 MeetAI 内部配置后，建议将 `.meetai/shims` 加入 PATH（默认路径为 `<meetai.exe目录>/.meetai/shims`），之后终端中的 `python` 命令就会跟随 `meetai python use` 自动切换，无需每次手动操作（详见上文 “3.1”）。
6. Node.js 已支持统一命令入口（`runtime` 与 `node`）；自动下载安装目前仅支持 Windows，macOS / Linux 需先手动安装后再切换使用。
7. Java / Go 仍处于规划阶段，当前版本会给出手动安装提示。
...
本项目以 **AGPL-3.0-only** 开源。

**个人使用、学习、二次开发**：完全自由，改了代码记得也开源出来。
**基于本项目对外提供网络服务**：需向你的用户公开对应源代码，闭源商用不在许可范围内。

详细条款见仓库根目录 [`LICENSE`](LICENSE) 文件。

## 参与贡献

欢迎一起把 MeetAI 做得更好，贡献方向优先级供参考：

- 从 `runtime` 子命令入手，补齐 Java / Go 安装器与 Node.js 跨平台安装能力。
